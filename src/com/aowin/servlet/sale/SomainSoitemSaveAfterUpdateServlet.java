...
public class SomainSoitemSaveAfterUpdateServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			boolean flag=sd.update(sm, ls,soId);
			if(flag){
				req.setAttribute("warning", "updateSuccess");
				List<Somain> cl=sd.getSomain();
				req.setAttribute("cl", cl);
				req.getRequestDispatcher("/sale/sale.jsp").forward(req, resp);
			}
			else{
				req.setAttribute("warning", "updateFail");
				List<Somain> cl=sd.getSomain();
				req.setAttribute("cl", cl);
				req.getRequestDispatcher("/sale/sale.jsp").forward(req, resp);
			}
